...
| **N tasks sent → N² tasks created** (e.g. 2 sent → 4 created, 3 sent → 9 created, 4 sent → 16 created) | **Unconfirmed/not yet fixed (2026-06-16).** The "For each task" loop runs the correct N times (confirmed via run history), but each iteration creates N tasks instead of 1 — classic signature of an **implicit nested Apply to each**. This happens when a field inside "Create To Do task" (Title, Due date, Body) is bound to the raw array-level token from "Parse task JSON" output instead of the current loop item — PA auto-wraps that field reference in its own hidden loop, nested inside the explicit "For each task" loop, giving N × N creations. | Open "Create To Do task" inside the loop. For each field (Title, Due date time, Body content), check the token isn't pulling directly from `Parse task JSON`'s array output (it'll show a small list/array icon). Delete and re-add each token by selecting it from `items('For_each_task')?['...']` — i.e. from the loop's current-item dynamic content, not the raw parsed array. Confirmed root-cause guess only — **not yet verified or fixed**; re-test with a small batch (e.g. 2 tasks) after applying and confirm 2 created, not 4. |
