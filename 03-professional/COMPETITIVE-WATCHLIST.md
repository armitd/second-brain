...
#### MuleSoft Agent Fabric — Deep Dive (July 2026)
- **What it is:** MuleSoft's specific implementation of the Agent Fabric control plane (branded "MuleSoft Agent Fabric," introduced Sept 2025, expanding through 2026) — discover, govern, orchestrate, and observe any agent or MCP server from one place, addressing "agent sprawl." Same shape as Microsoft Agent 365, but sits on top of Belron's existing Salesforce/MuleSoft estate rather than a cold vendor relationship.
- **Core components:** **Agent Registry** (centralised agent/MCP catalogue — "Anypoint Exchange for your digital workforce"); **Agent Scanners** (automated continuous discovery across Bedrock, Vertex AI, Azure AI Foundry, GoDaddy); **Agent Broker** (orchestration; new "Agent Script" for deterministic hand-off rules — beta April 2026, GA June 2026); **AI Gateway** (multi-vendor LLM cost/token/data governance, GA); **MCP Bridge** (makes existing APIs agent-ready via MCP with no code changes, GA); **Trusted Agent Identity** (scoped permissions + mobile approval for high-risk actions); **Agent Visualizer** (drag-and-drop workflow canvas, full GA June 2026).
- **MCP-specific relevance:** MCP Bridge is a direct, concrete candidate for turning Belron's existing legacy APIs into MCP tools without a bespoke build — directly relevant to the open CCOTF knowledge-base "buy vs. build" question (see 8 July CCOTF braindump). MCP server support ships May 2026.
- **Novel find — GoDaddy ANS integration:** GoDaddy's Agent Name Service acts as "DNS for AI agents" — verified, cryptographic agent identity published to the public DNS, discoverable across the open web. MuleSoft Agent Scanners pull verified agents from ANS into the Registry; once validated, an agent gets an Identity Certificate underpinning trusted Agent-to-Agent (A2A) communication. First cross-*organisation* (not just cross-vendor) agent trust mechanism seen across any tracked governance vendor.
- **Caveats (independent, via InfoWorld):** Informatica-hosted MCP data-quality checks add latency (ms to seconds); legacy APIs behind MCP Bridge need rigorous edge-case testing; an analyst quoted warns switching costs rise materially as orchestration/governance rules deepen inside Agent Fabric — a lock-in risk to weigh against the convenience of it sitting inside the existing Salesforce estate.
- **Pricing:** not disclosed in any source checked.
- **Belron relationship confirmed (2026-07-29):** Belron has an existing MuleSoft *and* Salesforce relationship — this is not a cold vendor evaluation. Sharpens the priority: Agent Fabric access is likely a commercial/licensing conversation with an existing vendor, not a net-new procurement decision. Still need to confirm the specific MuleSoft licence tier and whether Agent Fabric is bundled or a separate add-on.
- **Questions to answer:** Does Agent Fabric come bundled with Belron's existing MuleSoft/Anypoint licence tier, or as a separate purchase? Has the June 2026 full-GA timeline actually landed, or slipped (same verification discipline as Gemini 3.5 Pro)?
- **Relevance:** MCP Governance (third governance vendor to compare alongside Microsoft Agent 365 and Noma — the only one sitting on infrastructure Belron already licenses); Contact Centre of the Future (MCP Bridge as a KB reference-architecture candidate).
- **Source:** [[04-projects/mcp-governance/braindumps/braindump-2026-07-29-1423-mulesoft-agent-fabric]]
