# Why AgentKit Creates the Market for Production Agent Tools

![rw-book-cover](https://chrismdp.com/assets/img/wardley-map-openai-devday.png)

## Metadata
- Author: [[Chris Parsons]]
- Full Title: Why AgentKit Creates the Market for Production Agent Tools
- Category: #articles
- Summary: OpenAI’s AgentKit makes prototyping agents fast and cheap, so more teams will try agents. Once prototypes hit limits, those teams will need custom tools for reliability, security, cost, and scale. AgentKit expands the market for production agent platforms rather than killing it.
- URL: https://www.chrismdp.com/openai-devday-where-value-lives-in-ai-agent-tooling/
...
![](https://usf.rightmessage.com/481724109/qfZADamnQ0F9Wv0eAtFq5OZ2UhHC0v3O3xVtMGbP.png)
#### What OpenAI Actually Released

[AgentKit](https://openai.com/index/introducing-agentkit/) is OpenAI’s complete toolkit for agent development. Sam Altman positioned it as “a complete set of building blocks available in the OpenAI platform designed to help you take agents from prototype to production.”
...
Enhanced Evals now includes datasets, trace grading, automated prompt optimisation, and third-party model support. The automated prompt optimisation is particularly interesting - OpenAI confirmed they are using GEPA (Gradient-free Evolution for Prompt Adaptation), which was new to me. This validates the approach I have been building into [Kaijo](https://www.chrismdp.com/kaijo/) - using evaluation data to automatically improve prompts rather than manual prompt engineering. Carlyle reported that “the evaluation platform cut development time on our multi-agent due diligence framework by over 50%, and increased agent accuracy 30%.”
...
Understanding where this positions OpenAI requires mapping the AI agent tooling value chain. I built a Wardley map to analyse how AgentKit reshapes this landscape.[1](https://www.chrismdp.com/openai-devday-where-value-lives-in-ai-agent-tooling/#fn:wardley)
...
![Wardley Map: OpenAI DevDay Impact on AI Agent Tooling](https://www.chrismdp.com/assets/img/wardley-map-openai-devday.png)Wardley Map: OpenAI DevDay Impact on AI Agent Tooling
...
This is why AgentKit does not concern me for [Kaijo](https://www.chrismdp.com/kaijo/). Kaijo targets developers who are happy building traditional systems and do not want to think about the AI bit. They want to write functions, add tests, and let the platform handle prompt optimisation and model selection. With AgentKit, AI is all you can think about. You are constantly working within their workflow paradigm, their visual builder, their constraints.
...
**Network effects: the n8n integration moat.** [n8n](https://n8n.io) has built 400+ connectors to various services and platforms. Each connector represents weeks of development time to understand APIs, handle edge cases, and maintain compatibility across versions. Building a competitive connector library would require years and significant capital.
...
Fully AI-written workflows remain three to five years away. Agent Builder assists with implementation but does not make architectural decisions. As I discussed analysing [independent coding agents](https://www.chrismdp.com/independent-coding-agents-tools-arent-ready/), the infrastructure for autonomy is not ready.
...
For more insights on building production AI systems, join my [monthly webinar](https://www.chrismdp.com/webinar/) where I explore these topics with technical leaders.
